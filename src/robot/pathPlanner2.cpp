...
void getSetPointJoystick(){
    theta = atan2(joyData.joyX,joyData.joyY);
    ...
    magnitude = rr / max_r;
    desiredVelBL = magnitude * (sin(theta)+cos(theta)/ turn_damping);
    desiredVelFL = desiredVelBL;
    desiredVelBR = magnitude * (sin(theta) - cos(theta) / turn_damping);
