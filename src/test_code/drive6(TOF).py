...
arduino = serial.Serial(port='/dev/cu.usbserial-0264FEA5', baudrate=115200, timeout=.1)
...
def main():
    car = Car()
    while [car.x0, car.y0, car.heading0] == [None, None, None, None]: # wait until readArduino receives usable data
    ...
    while True:
        ...
        if [car.x_raw, car.y_raw, car.heading_raw, car.distance_raw] != [None, None, None, None] and (time.time() - car.prev_time) > 1e-3: 
            ...
            elif car.distance < CONE_DIST: 
                car.prev_state = car.state
                car.state = 6
            elif car.state == 0: ## go to AED waypoint
                car.target_x = 1.5
                car.target_y = 1.65
                car.go()
                if car.mini_state == 2:
...
class Car(object): 
    ...
    def avoid_cone(self):
        '''
        updates avoidx and avoidy based on state
        state 0: approaching AED waypoint, turn left
        state 5: approaching AED dropoff, turn right
        '''
        print('avoiding cone')
        if self.state == 0: 
            self.avoidx = self.x 
            self.avoidy = self.y + 0.2
            pass
        if self.state == 5: 
            self.avoidx = self.x
            self.avoidy = self.y - 0.2
